...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=50, os=16, pretrained=True, freeze_bn=False, _print=True):
        if _print:
        ...
        self.last_conv = nn.Sequential(nn.Conv2d(304, 1024, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(1024),
                                       nn.ReLU(),
                                       nn.Conv2d(1024, 1024, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(1024),
                                       nn.ReLU(),
                                       nn.Conv2d(1024, n_classes, kernel_size=1, stride=1))
    ...
    def forward(self, input):
        x, low_level_features = self.resnet_features(input) # torch.Size([1, 2048, 32, 32]) torch.Size([1, 256, 128, 128])
        x_low = x.clone()
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
